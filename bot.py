...
async def start(message: Message):
    await message.answer("Hi !!!", reply_markup=start_game_kb)
...
async def start_quiz(message: Message, state: FSMContext):
    ...
    await message.answer(
        "To stop the game send /stop_quiz command",
...
async def stop_quiz(message: Message, state: FSMContext):
    await state.finish()
    await message.answer("Stopped", reply_markup=start_game_kb)
...
async def show_answer(message: Message, state: FSMContext):
    data = await state.get_data()
    await message.answer(f"Correct answer was: {data['correct_answer']}")
    task = await get_new_task(state)
